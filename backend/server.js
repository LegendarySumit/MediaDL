...
const CAPTCHA_TRUST_TTL_MINUTES = Number(
  process.env.CAPTCHA_TRUST_TTL_MINUTES || 30,
);
const CAPTCHA_TRUST_TTL_MS = CAPTCHA_TRUST_TTL_MINUTES * 60 * 1000;
const QUEUE_DISABLED =
  process.env.DISABLE_QUEUE === "true" || process.env.NODE_ENV === "test";
...
const CAPTCHA_PROVIDER_CONFIGURED = Boolean(
  TURNSTILE_SECRET_KEY || RECAPTCHA_SECRET_KEY,
);
...
const PROD_FRONTEND_ORIGIN =
  process.env.FRONTEND_ORIGIN || "https://media-dl.vercel.app";
...
function splitPool(raw) {
  ...
  if (trimmed.startsWith("[")) {
    try {
      ...
      logger.warn(
        "Failed to parse JSON pool config; falling back to comma split",
      );
    }
  }
  return trimmed
...
function materializeCookieStringToFile(cookieText, label) {
  const filePath = path.join(
    os.tmpdir(),
    `yt_cookie_${label}_${crypto.randomBytes(6).toString("hex")}.txt`,
  );
...
function buildCookiePool() {
  ...
  cookiePoolRaw.forEach((entry, index) => {
    try {
      ...
      logger.error(
        { err: error, index },
        "Failed to materialize cookie pool entry",
      );
    }
  });
...
function classifyYtDlpError(message) {
  ...
  if (lowered.includes("tunnel connection failed: 402 account is inactive")) {
    return {
      ...
      httpStatus: 503,
      userMessage:
        "Proxy provider account is inactive. Please reactivate or remove proxy configuration.",
      retryable: false,
    };
  }
  ...
  if (
    ...
    return {
      ...
      httpStatus: 502,
      userMessage:
        "Upstream proxy connection failed while fetching media info.",
      retryable: true,
    };
  }
...
async function requireCaptcha(req, res, next) {
  if (!CAPTCHA_ENFORCED) {
    if (CAPTCHA_REQUIRED && !CAPTCHA_PROVIDER_CONFIGURED) {
      logger.warn(
        "CAPTCHA_REQUIRED=true but no provider secret configured; allowing request (fail-open)",
      );
    }
    return next();
  }
...
function buildYtArgs(base, options) {
  const args = [
    ...
    "1",
    "--extractor-args",
    `youtube:player_client=${options.playerClient || "web"}`,
    ...base,
  ];
  ...
  for (const attempt of attempts) {
    ...
    if (attempt.proxy && shouldSkipRemainingProxyAttempts(classified)) {
      ...
      logger.warn(
        ...
        "Skipping remaining proxy attempts for yt-dlp extraction",
      );
    }
    logger.warn(
      {
...
// Faster extraction for info endpoint: run multiple attempts in parallel
async function runYtDlpFast(extraArgs, timeoutMs = 5000) {
  const attempts = getAttemptConfigs();
  let fastestSuccess = null;
  let allFailed = [];

  // Run first 3 attempts in parallel for speed
  const promises = attempts.slice(0, 3).map((attempt) => {
    return new Promise((resolve) => {
      ...
      const onClose = (code) => {
        ...
        resolve({ ok: false, error: timedOut ? "timeout" : `exit ${code}` });
      };
  ...
  for (const result of results) {
    ...
    allFailed.push(result.error);
  }

  // If all fast attempts failed, try remaining sequential attempts
  for (let i = 3; i < attempts.length; i += 1) {
    ...
    const output = await new Promise((resolve) => {
      ...
      const cleanup = () => {
        child.stdout.removeListener("data", (chunk) => {
          stdout += chunk.toString();
        });
        child.stderr.removeListener("data", (chunk) => {
          stderr += chunk.toString();
        });
        child.removeListener("error", () => {});
        child.removeListener("close", () => {});
      };
  ...
  throw new Error(`All extraction attempts failed: ${allFailed.join(", ")}`);
}
  ...
  for (let attemptNo = 0; attemptNo < attempts.length; attemptNo += 1) {
    ...
    const result = await new Promise((resolve) => {
        ...
        for (const line of lines) {
          ...
          if (percentMatch) {
            const value = Math.max(
              0,
              Math.min(99, Math.floor(Number(percentMatch[1]))),
            );
      ...
      const onClose = (code) => {
        ...
        if (timedOut) {
          resolve({
            ok: false,
            error: `yt-dlp timed out after ${YTDLP_DOWNLOAD_ATTEMPT_TIMEOUT_MS}ms`,
    ...
    if (result.ok) {
      ...
      emitJobUpdate(job.id, {
        ...
        mimeType: getMimeType(
          path.extname(finalPath).replace(".", "") || desiredExt,
        ),
    ...
    if (attempt.proxy && shouldSkipRemainingProxyAttempts(classified)) {
      ...
      logger.warn(
        ...
        "Skipping remaining proxy attempts for download job",
      );
    }
    logger.warn(
      {
...
function initQueue() {
  ...
  worker = new Worker(QUEUE_NAME, async (job) => processDownloadJob(job), {
...
app.get("/api/image-proxy", async (req, res) => {
  ...
  try {
    ...
    return res
      .status(status)
      .json({ error: "Failed to fetch thumbnail image" });
  } finally {
    clearTimeout(timer);
  }
});
...
app.get("/api/info", async (req, res) => {
  ...
  try {
    ...
    const FAST_EXTRACTION_TIMEOUT = 5000;
    let raw;
    try {
      raw = await runYtDlpFast(
        ["--dump-json", "--no-playlist", url],
        FAST_EXTRACTION_TIMEOUT,
      );
    } catch (error) {
      // If extraction fails within 5 seconds, return degraded info with presets
      logger.info(
        { url, platform, error: error.message },
        "Info extraction failed; returning degraded info",
      );
    ...
    const bestAudioId = audioStreams[0]
